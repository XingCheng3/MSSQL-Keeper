...
public class BackupFileRepository : IBackupFileRepository
{
    ...
    public async Task<int> InsertAsync(BackupFile file)
    {
        ...
        return await db.ExecuteScalarAsync<int>("""
            INSERT INTO backup_files (task_id, execution_log_id, source_type, source_name, database_name, file_name, file_path, file_size_bytes, backup_type, created_at, expires_at, is_verified, status)
            VALUES (@TaskId, @ExecutionLogId, @SourceType, @SourceName, @DatabaseName, @FileName, @FilePath, @FileSizeBytes, @BackupType, @CreatedAt, @ExpiresAt, @IsVerified, @Status);
